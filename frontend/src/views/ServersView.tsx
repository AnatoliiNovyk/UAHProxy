...
import { Server as ServerIcon, Plus, Terminal, CheckCircle2, Shield, AlertCircle, Key, Trash2, Globe, RefreshCw } from 'lucide-react';
...
export const ServersView: React.FC<ServersViewProps> = ({ lang, servers, onRefresh }) => {
  ...
  const [testResult, setTestResult] = useState<{ [id: number]: { message: string; success: boolean } }>({});
  ...
  const handleTestSsh = async (serverId: number) => {
    ...
    try {
      ...
      setTestResult((prev) => ({
        ...
        [serverId]: {
          success: res.success,
          message: res.success ? `SSH OK: ${res.output}` : `SSH Failed: ${res.error || res.ssh_error_message || 'Connection failed'}`
        }
      }));
      onRefresh();
    } catch (e: any) {
        ...
        <div className="glass-panel rounded-2xl overflow-hidden border border-gray-800">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-800/60 text-sm">
              {servers.map((s) => {
                const liveRes = testResult[s.id];
                const isOnline = liveRes ? liveRes.success : s.ssh_status === 'ONLINE';
                const isOffline = liveRes ? !liveRes.success : s.ssh_status === 'OFFLINE';
                const errorMsg = liveRes ? liveRes.message : s.ssh_error_message;
                ...
                return (
                  <tr key={s.id} className="hover:bg-gray-800/30 transition">
                    ...
                    <td className="p-4">
                      {isOnline ? (
                        <span className="inline-flex items-center space-x-1.5 text-xs text-emerald-400 font-mono bg-emerald-950/60 px-2.5 py-1 rounded-md border border-emerald-800">
                          <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                          <span>Online (Connected)</span>
                        </span>
                      ) : isOffline ? (
                        <div className="space-y-1">
                          <span className="inline-flex items-center space-x-1.5 text-xs text-rose-400 font-mono bg-rose-950/80 px-2.5 py-1 rounded-md border border-rose-800">
                            <AlertCircle className="w-3.5 h-3.5 text-rose-400 flex-shrink-0" />
                            <span>Offline / SSH Failed</span>
                          </span>
                          {errorMsg && (
                            <div className="text-[11px] font-mono text-rose-300/80 max-w-sm truncate" title={errorMsg}>
                              {errorMsg}
                            </div>
                          )}
                        </div>
                      ) : (
                        <span className="inline-flex items-center space-x-1.5 text-xs text-gray-400 font-mono bg-gray-900 px-2.5 py-1 rounded-md border border-gray-800">
                          <span>Unchecked</span>
                        </span>
                      )}
                    </td>
            ...
            <form onSubmit={handleAddServer} className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <input
                    type="password"
                    placeholder="Введіть пароль вузла"
                    value={sshPassword}
              ...
              <div className="flex justify-end space-x-3 pt-4">
                ...
                >
                  {isSubmitting ? 'Збереження...' : 'Зберегти Вузол'}
                </button>
              </div>
            </form>
          </div>
